...
export function initLogin(container) {
    ...
    container.innerHTML = `
        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; padding: 20px; background: #05020a;">
            <div class="glass-box" style="width: 100%; max-width: 400px; padding: 40px; text-align: center; border-radius: 28px; background: rgba(88, 101, 242, 0.05); border: 1px solid rgba(88, 101, 242, 0.2); backdrop-filter: blur(20px); box-shadow: 0 20px 50px rgba(0,0,0,0.5);">
                ...
                <button id="login-discord" style="width: 100%; padding: 18px; border-radius: 14px; font-weight: 800; cursor: pointer; background: #5865F2; border: none; color: white; display: flex; align-items: center; justify-content: center; gap: 12px; transition: 0.3s; font-size: 16px; margin-bottom: 12px;">
                ...
                <button id="login-guest" style="width: 100%; padding: 15px; border-radius: 14px; font-weight: 700; cursor: pointer; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.2); color: white; transition: 0.3s; font-size: 14px;">
    ...
    document.getElementById('login-discord').onclick = async () => {
        ...
        await supabase.auth.signInWithOAuth({
            provider: 'discord',
            options: { redirectTo: siteUrl, scopes: 'identify email guilds' }
        });
    };
