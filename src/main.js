...
function renderPortal(user) {
    ...
    appContainer.innerHTML = `
        <div class="entry-container fade-in" id="entry-screen" style="display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100dvh; cursor: pointer; -webkit-tap-highlight-color: transparent; transition: opacity 0.4s ease;">
...
function checkAccess(user, container) {
    ...
    if (isVerified) {
        ...
        // FIX: min-height 100dvh per la pagina di check. Inseriti tap-highlight transparent ai bottoni.
        container.innerHTML = `
            <div class="fade-in" style="display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100dvh; text-align: center; padding: 30px;">
                ...
                <a href="${SERVER_INVITE}" target="_blank" class="btn-back-glass" style="background: #5865F2; border: none; padding: 20px 40px; text-decoration: none; -webkit-tap-highlight-color: transparent; outline: none;">UNISCITI AL SERVER</a>
                <button id="verify-btn" style="margin-top: 30px; background: transparent; border: 1px solid rgba(255,255,255,0.2); color: white; padding: 12px 25px; border-radius: 12px; cursor:pointer; font-size: 10px; letter-spacing: 2px; -webkit-tap-highlight-color: transparent; outline: none;">SONO GIÀ DENTRO</button>
            </div>
        `;
