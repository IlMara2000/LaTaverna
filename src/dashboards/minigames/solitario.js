...
export function initSolitario(container) {
    ...
    // FIX: Configurazione mobile-friendly per prevenire rimbalzi durante il drag
...
const quitGame = async (container) => {
    ...
    try {
        // FIX: Import dinamico corretto per tornare alla lista minigiochi senza ricaricare
...
function renderLayout(container, state) {
    ...
    <style>
        .solitario-wrapper { 
            ...
            animation: cardEntrance 0.6s forwards; box-sizing: border-box;
        }
        ...
        .slot { width: 52px; height: 78px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.03); position: relative; display: flex; align-items: center; justify-content: center; }
        .card-s { width: 52px; height: 78px; border-radius: 8px; position: absolute; box-shadow: 0 4px 8px rgba(0,0,0,0.5); border: 1px solid rgba(0,0,0,0.1); display: flex; flex-direction: column; padding: 4px; font-family: 'Montserrat', sans-serif; cursor: pointer; user-select: none; box-sizing: border-box; }
        ...
        .tableau-area { display: flex; justify-content: space-between; flex: 1; align-items: flex-start; overflow-y: auto; padding-bottom: 20px; }
        .col { width: 52px; height: 100%; position: relative; min-height: 100px; }
        .card-val-ui { font-size: 14px; font-weight: 900; line-height: 1; }
        ...
        .btn-action-sol { flex: 1; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: white; padding: 12px; border-radius: 12px; font-size: 11px; font-weight: 800; cursor: pointer; outline: none; }
    </style>
...
function renderCardHTML(card, index = 0, isTableau = false, isSelected = false) {
    ...
    return `<div class="card-s face-up ${selClass}" ${style}><div class="card-val-ui" style="color:${card.color}">${card.val}</div><div class="card-suit-small" style="color:${card.color}">${card.icon}</div><div class="card-suit-main" style="color:${card.color}">${card.icon}</div></div>`;
}
...
function renderGame(state, container) {
    ...
    if (state.waste.length > 0) {
        ...
        cardEl.onclick = (e) => { e.stopPropagation(); state.selected = isSelected ? null : { type: 'waste' }; renderGame(state, container); };
    ...
    state.tableau.forEach((col, colIdx) => {
        ...
        col.forEach((card, cardIdx) => {
            const isSelected = state.selected?.type === 'tableau' && state.selected.colIdx === colIdx && cardIdx >= state.selected.cardIdx;
            const cardWrapper = document.createElement('div');
            cardWrapper.innerHTML = renderCardHTML(card, cardIdx, true, isSelected);
            const cardEl = cardWrapper.firstElementChild;
            if (card.faceUp) {
                cardEl.onclick = (e) => { e.stopPropagation(); handleTableauClick(colIdx, cardIdx, state, container); };
...
function drawCard(state) {
    state.selected = null;
    if (state.deck.length === 0) { state.deck = state.waste.reverse().map(c => ({...c, faceUp: false})); state.waste = []; }
    else { const card = state.deck.pop(); card.faceUp = true; state.waste.push(card); }
}
...
function handleTableauClick(colIdx, cardIdx, state, container) {
    ...
    if (state.selected) {
        ...
        let canMove = false;
        if (!targetCard) { if (movingCard.rank === 13) canMove = true; }
        else if (targetCard.faceUp) { if (targetCard.isRed !== movingCard.isRed && targetCard.rank === movingCard.rank + 1) canMove = true; }
        if (canMove) {
    ...
    } else if (cardIdx !== -1 && col[cardIdx].faceUp) state.selected = { type: 'tableau', colIdx, cardIdx };
    renderGame(state, container);
}
...
function handleFoundationClick(fIdx, state, container) {
    if (state.selected) {
        const moveData = state.selected;
        const sourceStack = moveData.type === 'waste' ? state.waste : state.tableau[moveData.colIdx];
        if (moveData.type === 'tableau' && moveData.cardIdx !== sourceStack.length - 1) { state.selected = null; renderGame(state, container); return; }
