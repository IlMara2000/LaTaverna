...
export function initScacchi(container) {
    ...
    // FIX: Configurazione mobile-friendly per prevenire rimbalzi
...
function renderSetupMenu(container, state) {
    ...
    <style>
        ...
        .diff-btn { padding:10px; border-radius:8px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); color:white; font-size:10px; cursor: pointer; outline: none; }
        .diff-btn.active { background:rgba(157,78,221,0.2); border:1px solid #9d4ede; }
        .start-btn { width: 100%; padding: 16px; border-radius: 14px; border: none; background: #00ffa3; color: #000; font-weight: 900; font-size: 1.1rem; cursor: pointer; margin-top: 20px; outline: none; }
    </style>
    ...
    <div class="strategy-wrapper">
        <div class="setup-card">
            ...
            <p style="opacity:0.5; font-size:11px; margin-bottom:25px;">Chess & Checkers</p>
        ...
        <button id="btn-quit-setup" class="btn-back-glass" style="margin-top: 20px; width: 100%; padding: 15px; font-size: 12px;">← ESCI DAL GIOCO</button>
    </div>
    `;
...
function renderBoard(container, state) {
    ...
    <style>
        .game-screen { 
            ...
            animation: cardEntrance 0.4s ease-out forwards; box-sizing: border-box;
        }
        @media (min-width: 431px) {
        ...
        .board { 
            ...
            width: 90vw; max-width: 380px; height: 90vw; max-height: 380px;
            border: 2px solid rgba(255,255,255,0.05); border-radius: 8px; overflow: hidden;
        }
        .sq { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; font-size: 28px; position: relative; cursor: pointer; }
        .sq.light { background: #d1d9e0; }
        .sq.dark { background: #5c7c99; }
        .sq.selected { background: #f6f669 !important; }
        .piece { user-select: none; color: black; pointer-events: none; }
        .checker { width: 70%; height: 70%; border-radius: 50%; border: 2px solid rgba(0,0,0,0.2); }
        .checker.w { background: #fff; }
        .checker.b { background: #222; }
        .game-header { width: 100%; padding: 20px; display: flex; justify-content: space-between; align-items: center; position: absolute; top: env(safe-area-inset-top, 10px); }
        #btn-reset { position: absolute; bottom: calc(25px + env(safe-area-inset-bottom)); background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: white; padding: 10px 20px; border-radius: 12px; font-size: 11px; font-weight: 800; cursor: pointer; outline: none; }
    </style>

    <div class="game-screen">
        <div class="game-header">
            <span style="font-weight:900; font-size:14px;">${state.gameMode.toUpperCase()}</span>
            <div style="padding: 6px 12px; border-radius: 20px; font-size: 10px; font-weight: 800; background: ${state.turn === 'w' ? '#00ffa3' : '#ff416c'}; color: #000;">
...
function aiMove(state, container) {
    ...
    if (moves.length > 0) {
        ...
        if (m.tr < 8) { state.board[m.tr][m.tc] = state.board[m.fr][m.fc]; state.board[m.fr][m.fc] = null; }
    }
    state.turn = 'w';
