...
window.mockMultiplayer = function() {
    alert("🛠️ Server Multiplayer in costruzione! Interfaccia pronta per Socket.io.");
}
...
function animateCardDeal(targetIndex, duration = 0.4, callback = null) {
    ...
    flyingCard.getBoundingClientRect(); // trigger reflow
...
function nextTurn(skip = false) {
    ...
    // Formula circolare basata sulla direzione
...
function handleCardEffects(card) {
    ...
    switch (card.value) {
        case 'cambio_giro': 
            ...
            if (gameState.numPlayers === 2) nextTurn(true);
            break;
...
function penalizePlayer(pIdx, amount) {
    ...
    let intv = setInterval(() => {
        ...
        count++;
        if (count >= amount) { clearInterval(intv); setTimeout(() => { isAnimating = false; }, 400); }
    }, 200);
}
    ...
    container.innerHTML = `
        <style>
            .solo-card {
                ...
                font-weight: 900; font-size: 1.4rem; cursor: pointer; transition: 0.3s; box-shadow: 0 4px 10px rgba(0,0,0,0.5);
            ...
            .bot-pos { position: absolute; display: flex; gap: 5px; align-items: center; padding: 10px; transition: 0.4s; border-radius: 15px; }
            .active-turn { box-shadow: 0 0 25px 5px var(--amethyst-bright); background: rgba(157, 78, 221, 0.15); }
            .flying-card {
                position: fixed; width: 60px; height: 90px; border-radius: 8px; background: linear-gradient(135deg, #331155, #110522);
                border: 2px solid var(--amethyst-bright); z-index: 9999; pointer-events: none; transition-property: top, left, transform, opacity;
            ...
            .direction-indicator { font-size: 20px; transition: 0.5s; position: absolute; top: -40px; }
        </style>
        ...
        <div class="fade-in" style="width:100%; height:100dvh; background: radial-gradient(circle at center, #1a0a2a 0%, #05020a 100%); position:relative; overflow:hidden; color:white;">
            ...
            <div id="start-overlay" style="position:fixed; inset:0; background:rgba(0,0,0,0.85); z-index:2000; display:flex; align-items:center; justify-content:center; backdrop-filter:blur(10px);">
                <div style="text-align:center; max-width: 320px; width: 90%;">
                    <h1 style="font-size:4rem; font-weight:900; color:var(--amethyst-bright); margin-bottom:30px;">SOLO</h1>
                    <div id="mode-selection">
                        <button class="main-btn" onclick="showModeSelection('single')">VS BOT (DIFFICILE)</button>
                        <button class="sec-btn" onclick="showModeSelection('multi')">MULTIPLAYER ONLINE</button>
                    </div>
                    <div id="multiplayer-ui" style="display:none;">
                        <input type="text" placeholder="CODICE STANZA" style="width:100%; padding:15px; border-radius:12px; border:1px solid var(--amethyst-bright); background:black; color:white; text-align:center; margin-bottom:15px;">
                        <button class="main-btn" onclick="mockMultiplayer()">UNISCITI</button>
                        <button class="sec-btn" onclick="backToModeSelection()">INDIETRO</button>
                    </div>
                </div>
            </div>

            <div style="padding:20px; display:flex; justify-content:space-between; width:100%; position:absolute; z-index:100;">
                <button id="exit-game" style="background:rgba(255,68,68,0.2); border:1px solid #ff4444; color:white; padding:8px 15px; border-radius:10px;">ESCI</button>
                <div id="turn-indicator" style="font-weight:900; color:var(--amethyst-bright); background:rgba(0,0,0,0.5); padding: 5px 15px; border-radius: 20px;"></div>
            ...
            <div style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); text-align:center;">
                <div id="dir-icon" class="direction-indicator" style="left:50%; transform:translateX(-50%);"></div>
                <div style="display:flex; gap:30px; align-items:center; justify-content:center; margin-bottom:20px;">
                    <div id="deck-element" onclick="playerDrawCard()" style="width:80px; height:120px; background:linear-gradient(135deg, #331155, #110522); border:2px solid var(--amethyst-bright); border-radius:12px; cursor:pointer;"></div>
                    <div id="discard-card" class="solo-card" style="width:90px; height:130px; cursor:default;"></div>
                </div>
                <div id="color-info" style="font-size:11px; letter-spacing:2px; background:rgba(0,0,0,0.5); padding:8px 15px; border-radius:20px;">
            ...
            <div style="position:absolute; bottom:20px; width:100%; text-align:center;">
                <button onclick="callSolo()" style="background:#ff4444; color:white; padding:12px 50px; border-radius:50px; font-weight:900; border:none; cursor:pointer;">SOLO!</button>
            </div>
...
function renderGameView() {
    ...
    document.getElementById('dir-icon').style.transform = `translateX(-50%) scaleX(${gameState.direction})`;
