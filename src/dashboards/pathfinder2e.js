    ...
    content.innerHTML = `
        <section class="dnd-grid">
            ...
            <button class="dnd-panel" data-open="manuals">
                ...
                <p>Cerca nei riferimenti Pathfinder oppure consulta ogni pagina disponibile.</p>
            </button>
...
async function renderSessions(container) {
    ...
    const loadSessions = async () => {
        ...
        try {
            ...
            const canShareSessions = !isLocalPathfinderUser(user);
            list.innerHTML = `
            ...
            list.querySelectorAll('[data-share-session]').forEach(btn => {
                btn.onclick = async () => {
                    ...
                    try {
                        const { url } = await ensureSessionInvite(session, 'pathfinder2e');
